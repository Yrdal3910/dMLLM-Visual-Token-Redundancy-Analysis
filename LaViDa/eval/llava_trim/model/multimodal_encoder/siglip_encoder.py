...
class SigLipVisionTower(nn.Module):
    """
    SigLIP vision tower with configurable feature selection and TRIM token
    reduction based on text-image similarity.
    """
    ...
    def _import_siglip_classes(self):
        try:
            from transformers import AutoModel, AutoTokenizer, SiglipTextModel
        except Exception as e:
            raise RuntimeError(f"Unable to import required SigLIP classes: {e}")
        return AutoTokenizer, AutoModel, SiglipTextModel

    def _init_text_side_mandatory(self, device_map=None):
        """
        Load the tokenizer and text tower required by TRIM.

        Loading failures are reported immediately because text features are
        mandatory for similarity-based token reduction.
        """
        AutoTokenizer, AutoModel, SiglipTextModel = self._import_siglip_classes()
        self.text_side_path = "/pfs/zuhaoyang/workspace/cache/siglip2-so400m-patch14-384"

        path = self.text_side_path or self.vision_tower_name
        if path is None:
            raise RuntimeError("A complete SigLIP path is required to initialize the text tower.")

        try:
            self.text_tokenizer = AutoTokenizer.from_pretrained(path, trust_remote_code=True)
        except Exception as e:
            raise RuntimeError(
                f"Failed to load the SigLIP tokenizer from {path}: {e}"
            )
        ...
        try:
            self.text_tower = SiglipTextModel.from_pretrained(
                path,
            ...
            whole = AutoModel.from_pretrained(
                path,
                torch_dtype=torch.float32,
                low_cpu_mem_usage=False,
                device_map=None
            )
            self.text_tower = getattr(whole, "text_model", None) or \
                            (getattr(whole, "get_text_model", lambda: None)())
            if self.text_tower is None:
                raise RuntimeError("Unable to extract the text model from the full SigLIP model.")

        if self.text_tower is None:
            try:
                self.text_tower = AutoModel.from_pretrained(path, subfolder="text_model",
                                                            trust_remote_code=True, device_map=device_map)
            except Exception as e:
                err_msgs.append(f"subfolder=text_model failed: {e}")

        if self.text_tower is None:
            raise RuntimeError(
                "Unable to load the SigLIP text model. Ensure the path contains "
                f"complete SigLIP weights, including text_model. Path: {path}. "
    ...
    def _encode_texts_chunked(self, texts, max_len=None):
        """
        Encode long texts in non-overlapping token windows and average the
        resulting chunk embeddings for each sample.
        """
        ...
        all_embeds = []
        for t in texts:
            ids = self.text_tokenizer(t, add_special_tokens=False)["input_ids"]

            chunks = [ids[i:i + chunk_body_len] for i in range(0, len(ids), chunk_body_len)] or [[]]

            chunk_embeds = []
            for ch in chunks:
                ch_with_specials = self.text_tokenizer.build_inputs_with_special_tokens(ch)

                input_ids = torch.tensor([ch_with_specials], dtype=torch.long, device=self.device)
                attn = torch.ones_like(input_ids)

                pad_id = self.text_tokenizer.pad_token_id or self.text_tokenizer.eos_token_id
                ...
                self.text_tower.eval().to(dtype=torch.float32)
                assert input_ids.device == next(self.text_tower.parameters()).device
                with torch.no_grad():
                    out = self.text_tower(input_ids=input_ids, attention_mask=attn, output_hidden_states=False)

                if hasattr(out, "text_embeds") and out.text_embeds is not None:
                    chunk_embeds.append(out.text_embeds)
                else:
                    cls = out.last_hidden_state[:, 0]
                    if hasattr(self.text_tower, "text_projection"):
                        chunk_embeds.append(self.text_tower.text_projection(cls))
                    else:
                        chunk_embeds.append(cls)
    ...
    def load_model(self, device_map=None):
        ...
        if hasattr(self.vision_tower, "config") and hasattr(self.vision_tower.config, "hidden_size"):
            vis_dim = self.vision_tower.config.hidden_size
        else:
            vis_dim = next(self.vision_tower.parameters()).shape[-1]

        txt_dim = vis_dim
        if hasattr(self, "text_tower"):
            txt_cfg = getattr(self.text_tower, "config", None)
            if txt_cfg is not None and hasattr(txt_cfg, "projection_size"):
                txt_dim = txt_cfg.projection_size
            elif hasattr(self.text_tower, "text_projection"):
                txt_dim = self.text_tower.text_projection.weight.shape[0]

        if vis_dim == txt_dim:
            self.visual_projection = nn.Identity()
        else:
            proj = nn.Linear(vis_dim, txt_dim, bias=False)
            nn.init.normal_(proj.weight, std=0.02)
            self.visual_projection = proj

        dev = next(self.vision_tower.parameters()).device
        self.post_layernorm.to(dev)
        self.visual_projection.to(dev)

        self._has_visual_projection = True
    ...
    def feature_select(self, image_forward_outs):
        ...
        exp_patches = self.num_patches
        sequence_length = all_image_features.shape[1]

        has_cls = sequence_length == exp_patches + 1
    ...
    def token_reduction(self, image_features, all_image_features, text_features=None):
        """
        Reduce visual tokens with TRIM.

        The most relevant patch tokens are retained and all remaining tokens
        are represented by one mean token.
        """
        if not self.token_reduce_func:
            B, T, _ = image_features.shape
            return image_features, [T] * B

        assert self.visual_projection is not None and self.post_layernorm is not None, \
            "TRIM requires visual_projection and post_layernorm."
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            feats_ln = image_features.to(dtype=ln_dtype)
            normed = self.post_layernorm(feats_ln)
            ...
            proj_in = normed.to(dtype=proj_dtype)
            proj_image_features = self.visual_projection(proj_in)

            if text_features is None:
                raise RuntimeError("TRIM requires text_features; pass texts to forward().")

            text_in = text_features.to(dtype=proj_image_features.dtype)
        ...
        if k == -1:
            def outlier_detection(sim):
                sim_np = sim.to(dtype=torch.float32).detach().cpu().numpy().flatten()
        ...
        num_tokens_to_keep = max(1, min(tokens_number, num_tokens_to_keep))

        _, topk_indices = torch.topk(similarities, num_tokens_to_keep, dim=1, largest=True, sorted=False)
        ...
        selected_image_features = selected_image_features.to(dtype=image_features.dtype)
    ...
    def forward(self, images, texts=None):
        """
        Return visual features and the actual token count for each sample.
        """
        text_features = None
        if self.token_reduce_func:

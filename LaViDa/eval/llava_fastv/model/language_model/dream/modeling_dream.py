...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if (
            ...
            and gu.PRUNED_LAYER_NUMS == gu.get_fastv_target_layer_call(self.num_hidden_layers)
            and gu.PRUNED_STEPS_NUMS == gu.FASTV_PRUNING_STEP
        ):
            try:
...
class DreamDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, attn_scores, position_ids, position_embeddings = self.self_attn(
            hidden_states=hidden_states,
            prune_num=prune_num,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        if gu.START_COMPRESSION_MODE and gu.is_fastv_pruning_point(
            gu.PRUNED_LAYER_NUMS,
            self.layer_idx,
            ...
            system_prompt_len = system_prompt_len.to(hidden_states.device)
            visual_seq_len = visual_seq_len.to(hidden_states.device)
            ...
            values, indices = torch.topk(token_attention, int(visual_seq_len * gu.keep_ratio), dim=-1)

            gu.VISUAL_INDICES = indices
            ...
            prune_num = torch.full((batch_size,), visual_seq_len.item() - hidden_states_visual.shape[1], device=device)

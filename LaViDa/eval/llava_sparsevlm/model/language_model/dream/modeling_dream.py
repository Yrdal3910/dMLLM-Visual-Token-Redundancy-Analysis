...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    attn_scores = None
    attn_scores = attn_weight.mean(dim=1)
    ...
    return attn_weight @ value, attn_scores
...
def adjust_position_ids(position_ids, prune_num) -> torch.Tensor:
    ...
    new_token_num = max_values + 1 - prune_num.squeeze(-1)                  #(batch_size)
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if (
            gu.START_COMPRESSION_MODE
            and gu.PRUNED_LAYER_NUMS == gu.get_sparsevlm_target_layer_call(self.num_hidden_layers)
            and gu.PRUNED_STEPS_NUMS == gu.SPARSEVLM_PRUNING_STEP
        ):
            # try:
            #     position_ids = adjust_position_ids(position_ids, prune_num)
            # except:
            #     position_ids = position_ids.to(prune_num.device)
            #     position_ids = adjust_position_ids(position_ids, prune_num)
            cos, sin = self.rotary_emb(value_states, position_ids)
        ...
        if not self.training:
            ...
            attn_output, attn_scores = scaled_dot_product_attention(
                query_states,
                key_states,
                value_states,
        ...
        return attn_output, None, past_key_value, attn_scores, position_ids, position_embeddings

class DreamFlashAttention2(DreamAttention):
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        return attn_output, attn_weights, past_key_value, attn_scores, position_ids, position_embeddings
...
DREAM_ATTENTION_CLASSES = {
    "flash_attention_2": DreamFlashAttention2,
...
class DreamDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        if gu.START_COMPRESSION_MODE and sparse_layer :   # 
            hidden_states, self_attn_weights, present_key_value, relation_vis_text, position_ids, position_embeddings = self.self_attn(
                hidden_states=hidden_states,
                prune_num=prune_num,
                policy=policy,
                attention_mask=attention_mask,
            ...
            gu.PRUNED_LAYER_NUMS += 1

            prune_num = torch.full((hidden_states.shape[0],), int(v_token_num * (1 - gu.PRUNING_RATIO)), device=prune_num.device)
            ...
            return outputs, prune_num, position_ids, position_embeddings

        else:
            hidden_states, self_attn_weights, present_key_value, attn_logits, position_ids, position_embeddings = self.self_attn(
                hidden_states=hidden_states,
                prune_num=prune_num,
                policy=policy,
                attention_mask=attention_mask,
...
class DreamBaseModel(DreamPreTrainedModel):#
    ...
    def __init__(self, config: DreamConfig, pruning_loc=None):
    ...
    def forward(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if (
                    gu.START_COMPRESSION_MODE
                    and gu.is_sparsevlm_pruning_point(
                        gu.PRUNED_STEPS_NUMS,
                        layer_idx,
                        self.pruning_loc,
                    )
                    ...
                    layer_outputs, prune_num, position_ids, position_embeddings = decoder_layer(
                        hidden_states,
                        prune_num=prune_num,
                    ...
                    pred_score_vis, s_flag, relation_vis_text = attn_postprocess_topk(attn_logits, v_token_start, v_token_num, text_token_start, t_token_idx, layer_idx, self.visual_seq_len.item() * gu.PRUNING_RATIO) # B, L_v

                    gu.VISUAL_INDICES = pred_score_vis
                    ...
                    if s_flag and total_sparse_token_idx.shape[1]>0:
                        ...
                        gu.MERGE_SPARSE_TOKEN =  merge_sparse_token

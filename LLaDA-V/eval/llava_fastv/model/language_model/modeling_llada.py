...
# FastV pruning schedule. Both values are zero-based:
# - step 47 means the 48th denoising/model forward pass.
# - layer 2 means pruning immediately after the third Transformer layer.
#
# PRUNED_LAYER_NUMS increases once per Transformer layer, so the cumulative
# counter at the pruning point is:
#   step * num_hidden_layers + layer + 1
FASTV_PRUNING_STEP = 47
FASTV_PRUNING_LAYER = 2

# Fraction of visual tokens retained at the configured pruning point.
PRUNING_RATIO = 0.25

# Indices selected by FastV inside the target layer. The generation loop uses
# the same indices to keep token IDs and all persistent embeddings aligned.
VISUAL_INDICES = 0


def get_fastv_target_layer_call(num_hidden_layers):
    """Return the cumulative layer count immediately after FastV pruning."""
    return (
        FASTV_PRUNING_STEP * num_hidden_layers
        + FASTV_PRUNING_LAYER
        + 1
    )


def is_fastv_pruning_point(layer_call_count, layer_idx, num_hidden_layers):
    """Return whether the current layer output should be pruned by FastV."""
    return (
        layer_call_count == get_fastv_target_layer_call(num_hidden_layers)
        and layer_idx == FASTV_PRUNING_LAYER
    )
...
def adjust_position_ids(position_ids, prune_num) -> torch.Tensor:

    assert position_ids.dim() == 2, f"Expected 2D tensor, but got {position_ids.dim()}D"

    batch_size, token_num = position_ids.shape

    assert batch_size == 1, f"batch_size must be 1, but got {batch_size}"

    max_values, _ = position_ids.max(dim=-1)
    new_token_num = max_values + 1 - prune_num.squeeze(-1)
    new_position_ids = torch.stack([torch.arange(num.item(), device=position_ids.device) for num in new_token_num])
...
class LLaDASdpaAttention(LLaDAAttention):
    ...
    def forward(
        ...
        global PRUNED_LAYER_NUMS, PRUNED_STEPS_NUMS
        # Tokens are removed after FASTV_PRUNING_LAYER. At the next layer's
        # attention call, shorten position IDs before applying RoPE.
        if (
            PRUNED_LAYER_NUMS
            == get_fastv_target_layer_call(self.num_hidden_layers)
            and PRUNED_STEPS_NUMS == FASTV_PRUNING_STEP
        ):
            position_ids = adjust_position_ids(position_ids, prune_num)
        ...
        return attn_output, None, past_key_value, attn_score, position_ids
...
class LLaDADecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, attn_scores, position_ids = self.self_attn(
        ...
        global PRUNED_LAYER_NUMS, PRUNING_RATIO, VISUAL_INDICES

        # Count completed Transformer layers across all denoising passes. This
        # counter starts at zero for each sample and is reset after generation.
        PRUNED_LAYER_NUMS += 1

        if is_fastv_pruning_point(
            PRUNED_LAYER_NUMS,
            self.layer_idx,
            self.num_hidden_layers,
        ):
            batch_size, token_nums, hidden_dimention = hidden_states.shape

            device = visual_seq_len.device
            system_prompt_len = system_prompt_len.item()
            visual_seq_len = visual_seq_len.item()

            hidden_states_prompt = hidden_states[:, :system_prompt_len, :]
            hidden_states_visual = hidden_states[:, system_prompt_len:system_prompt_len + visual_seq_len, :]
            hidden_states_text = hidden_states[:, system_prompt_len + visual_seq_len:, :]

            attn_scores = attn_scores[:, :, system_prompt_len:system_prompt_len + visual_seq_len]
            token_attention = attn_scores.sum(dim=1)
            values, indices = torch.topk(token_attention, int(visual_seq_len * PRUNING_RATIO), dim=-1)

            VISUAL_INDICES = indices

            indices_expanded = indices.unsqueeze(-1).expand(-1, -1, hidden_states_visual.size(-1))
            hidden_states_visual = torch.gather(hidden_states_visual, dim=1, index=indices_expanded)

            hidden_states = torch.cat([hidden_states_prompt, hidden_states_visual, hidden_states_text], dim=1)

            prune_num = torch.full((batch_size,), visual_seq_len - hidden_states_visual.shape[1], device=device)
        residual = hidden_states
        ...
        return outputs, prune_num, position_ids
...
class LLaDAModel(LLaDAPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        prune_num = torch.full((batch_size,), 0, device=self.visual_seq_len.device)
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs, prune_num, position_ids = decoder_layer(
                    hidden_states,
                    prune_num=prune_num,
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def _build_conversation_mask_optimized(self, conversation_ids):
        ...
        conv_mask = (ids_j <= ids_i)

        # Add the attention head dimension
        return conv_mask.unsqueeze(1)
    ...
    def generate_with_embeds(self, inputs_embeds, system_prompt_len: Optional[torch.Tensor] = None, image_token_length: Optional[torch.Tensor] = None, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        '''
        global PRUNED_LAYER_NUMS, PRUNED_STEPS_NUMS, PRUNING_RATIO, VISUAL_INDICES
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    # A complete model forward is one denoising step. Layer-level
                    # pruning occurs while this counter is FASTV_PRUNING_STEP.
                    # After the forward returns, apply the selected visual-token
                    # indices to all persistent generation states.
                    PRUNED_STEPS_NUMS += 1
                    if PRUNED_STEPS_NUMS == FASTV_PRUNING_STEP + 1:
                        ...
                        VISUAL_INDICES = VISUAL_INDICES.to(dtype=torch.long, device=x.device)

                        idx_expanded = VISUAL_INDICES.unsqueeze(-1).expand(-1, -1, x_embeds_visual.size(-1))
                        x_embeds_visual = torch.gather(x_embeds_visual, dim=1, index=idx_expanded)
                        inputs_embeds_visual = torch.gather(inputs_embeds_visual, dim=1, index=idx_expanded)

                        x_visual = torch.gather(x_visual, dim=1, index=VISUAL_INDICES)
            ...
            # Counters are sample-local and must not leak into the next request.
            PRUNED_LAYER_NUMS = 0
            PRUNED_STEPS_NUMS = 0
    ...
    def forward(
        ...
        masked_indices = self.get_masked_indices_from_embeds(noisy_embeds, masked_embed)
        ...
        if conversation_ids is not None:
            ...
            if attention_mask is not None:
                ...
                attention_mask = attention_mask.expand_as(conversation_mask)

...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, is_llada=False):
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if mm_newline_position == "grid":
                            ...
                            if START_COMPRESSION_MODE:

                                token_nums, hidden_dimension = image_feature.size(0), image_feature.size(1)

                                k_keep = round(keep_ratio * token_nums)

                                image_feature_flat = image_feature   #[32*196, 4096]
                                # print(token_nums)

                                pivot_token_idx = torch.randint(0, token_nums, ()).to(image_feature_flat.device)

                                with torch.no_grad():
                                    # start = time.perf_counter()

                                    keep_idx = self.min_max_greedy_search(
                                        image_feature_flat, k_keep, pivot_token_idx
                                        )

                                    # end = time.perf_counter()
                                    #print(f"执行时间: {end - start:.6f} 秒")

                                image_feature = image_feature_flat[keep_idx]
                                # print(f"image_feature.size is: {len(image_feature)}", "\n")
                        ...
                        # print(max_num_patches)      # max_num_patches = 2
                        ...
                        # print(image_feature.shape)        # torch.Size([(5, 5) (2, 3) ..., 27, 27, 4096])
                        ...
                        if START_COMPRESSION_MODE:
                            ...
                            token_nums, hidden_dimension = image_feature.size(0), image_feature.size(1)
                            k_keep = round(keep_ratio * token_nums)
                            # print(f"k_keep is: {k_keep}; token_nums is: {token_nums}")

                            image_feature_flat = image_feature   #[n*729(any_res), 4096]
                            pivot_token_idx = torch.randint(0, token_nums, ()).to(image_feature_flat.device)
                            ...
                            with torch.no_grad():
                                ...
                                # print(keep_idx)
                                # keep_idx = [True for i in range(token_nums)]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    # 自己改的
                    image_token_length.append(cur_image_features.shape[0])
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, image_token_length

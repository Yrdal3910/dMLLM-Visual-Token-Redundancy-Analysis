...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    if attn_mask is not None:
        if attn_mask.dtype == torch.bool:
            ...
            attn_bias = attn_bias.unsqueeze(0).unsqueeze(0).to(device=attn_mask.device)
            
            #TODO: need to further consider
            attn_mask = attn_mask[..., :attn_bias.shape[-2], :]
            assert attn_bias.shape == attn_mask.shape, f"attn_bias.shape is {attn_bias.shape}, attn_mask.shape is {attn_mask.shape}"
    ...
    attn_weight = attn_weight.to(attn_bias.device)
    attn_weight += attn_bias
    attn_weight = torch.softmax(attn_weight, dim=-1)
    attn_weight = torch.dropout(attn_weight, dropout_p, train=True)
    
    attn_weight = attn_weight.to(value.device)
...
class LLaDASdpaAttention(LLaDAAttention):
    ...
    def forward(
        ...
        return attn_output, None, past_key_value, attn_score
...
class LLaDADecoderLayer(nn.Module):
    ...
    def plot_attention(self, attn_scores, layer_idx):
        ...
        cbar = plt.colorbar(label="Prefilling Attention Scores in LLM")

        cbar.set_ticks([1e-3, 1e-2, 1e-1, 1e0])
        cbar.set_ticklabels(['0.001', '0.01', '0.1', '1'])

        plt.title(f"Step{STEP}: Attention Scores of Layer{layer_idx}")
        plt.xlabel("Token Index")
        plt.ylabel("Token Index")
        plt.savefig(
            f"/pfs/zuhaoyang/workspace/attn_scores_step{STEP}_layer{layer_idx}.png",
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()

    def plot_attention_ratio(self, num_tokens, attn_scores, layer_idx, system_prompt_len, visual_seq_len, gen_length):

        prompt_len, visual_len = system_prompt_len.item(), visual_seq_len.item()
        text_len = num_tokens - prompt_len - visual_len - gen_length

        # print(f"prompt_len is: {prompt_len}, visual_len is: {visual_len}, text_len is: {text_len}, token_num is: {hidden_states.size(1)}")

        prmt, vis, txt, ans = slice(0, prompt_len), slice(prompt_len, prompt_len + visual_len), slice(prompt_len + visual_len, prompt_len + visual_len + text_len), slice(prompt_len + visual_len + text_len, num_tokens)

        attn_scores = attn_scores.to(torch.float32)
        # print(attn_score)

        num = attn_scores[:, ans, prmt].sum(dim=(1, 2))
        den = attn_scores[:, ans, :].sum(dim=(1, 2))
        alpha_ans2p = (num / den.clamp_min(1e-12)).mean()

        num = attn_scores[:, ans, vis].sum(dim=(1, 2))
        den = attn_scores[:, ans, :].sum(dim=(1, 2))
        alpha_ans2v = (num / den.clamp_min(1e-12)).mean()

        num = attn_scores[:, ans, txt].sum(dim=(1, 2))
        den = attn_scores[:, ans, :].sum(dim=(1, 2))
        alpha_ans2t = (num / den.clamp_min(1e-12)).mean()

        num = attn_scores[:, ans, ans].sum(dim=(1, 2))
        den = attn_scores[:, ans, :].sum(dim=(1, 2))
        alpha_ans2ans = (num / den.clamp_min(1e-12)).mean()

        print(f"Layer {layer_idx}: alpha_ans2p is {alpha_ans2p}, alpha_ans2v is {alpha_ans2v}, alpha_ans2t is {alpha_ans2t}, alpha_ans2ans is {alpha_ans2ans}")

    def plot_attention_distribution(self, attn_scores, layer_idx, steps, system_prompt_len, visual_seq_len):
        attn = attn_scores[0]

        # 计算每个 token 收到的总 attention
        received_attention = attn.sum(axis=0)[system_prompt_len : system_prompt_len + visual_seq_len]  # shape (v_n,)
        received_attention = received_attention.detach().cpu().numpy()

        # # 分布直方图
        # plt.figure(figsize=(6, 4))
        # plt.hist(received_attention, bins=20, density=True, alpha=0.6, color="green", label="Histogram")
        # plt.title(f"Step {steps} Layer {layer_idx}: Distribution of Received Attention per Token")
        # plt.xlabel("Total Received Attention")
        # plt.ylabel("Density")
        # plt.grid(alpha=0.3)
        # plt.legend()
        # plt.savefig(f"/pfs/zuhaoyang/workspace/Step{steps}_layer_{layer_idx}_attn_hist.png", dpi=300, bbox_inches="tight")
        # plt.close()

            # 过滤掉 <=0 或 >=10 的值
        received_attention = received_attention[(received_attention > 0) & (received_attention < 10)]

        # 如果没有数据，直接返回
        if received_attention.size == 0:
            return
        ...
        kde = gaussian_kde(received_attention, bw_method="scott")
        y = kde(x)

        # 绘制平滑曲线
        plt.figure(figsize=(6, 4))
        plt.plot(x, y, color="black", linewidth=2)
        plt.fill_between(x, y, alpha=0.2, color="gray")  # 阴影填充，可选
        plt.title(f"Step {steps} Layer {layer_idx}: KDE of Received Attention (0 < x < 10)")
        plt.xlabel("Total Received Attention")
        plt.ylabel("Density")
        plt.grid(alpha=0.3)
        plt.savefig(f"/pfs/zuhaoyang/workspace/Step{steps}_layer_{layer_idx}_attn_kde.png",
                    dpi=300, bbox_inches="tight")
        plt.close()

        # 每个 token 的柱状图
        plt.figure(figsize=(10, 4))
        plt.bar(range(len(received_attention)), received_attention, color="skyblue")
        plt.title(f"Step {steps} Layer {layer_idx}: Total Received Attention per Token")
        plt.xlabel("Token Index")
        plt.ylabel("Total Received Attention")
        plt.savefig(f"/pfs/zuhaoyang/workspace/Step{steps}_layer_{layer_idx}_attn_bar.png", dpi=300, bbox_inches="tight")
        plt.close()

    def forward(
        self,
        hidden_states: torch.Tensor,
        visual_seq_len: torch.Tensor,
        system_prompt_len: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        **kwargs,
        ...
        # Self Attention
        # hidden_states, self_attn_weights, present_key_value, attn_scores = self.self_attn(
        ...
        global PRUNED_STEPS_NUMS
        # check attention across token types and layers
        # self.plot_attention(attn_scores, self.layer_idx)
        # self.plot_attention_ratio(hidden_states.size(1), attn_scores, self.layer_idx, system_prompt_len, visual_seq_len, 2)      # 2 corresponds to "gen_length" 2
        # if PRUNED_STEPS_NUMS == 0:
        #     self.plot_attention_distribution(attn_scores, self.layer_idx, PRUNED_STEPS_NUMS, system_prompt_len.item(), visual_seq_len.item())
...
class LLaDAModel(LLaDAPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 自己加的
        batch_size, tokens_num, hidden_dimension = hidden_states.shape
        self.system_prompt_len = system_prompt_len
        self.visual_seq_len = image_token_length
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    visual_seq_len=self.visual_seq_len,
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    @torch.no_grad()
    def generate_with_embeds(self, inputs_embeds, system_prompt_len: Optional[torch.Tensor] = None, image_token_length: Optional[torch.Tensor] = None, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        '''
        global PRUNED_STEPS_NUMS, keep_ratio
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if FAST_SAMPLING:
                ...
                steps = steps * num_blocks  # 恢复调用方传入的原始 steps
                ...
                assert gen_length % steps == 0, "gen_length should be divisible by total steps"
                avg_per_step = math.ceil(gen_length / steps)
                k_pre = avg_per_step

                # ---- 状态 ----
                decoded_total = 0
                finalized_mask = torch.zeros_like(x, dtype=torch.bool, device=device)  # 只统计生成段的“已落锤”
                p_hist = deque(maxlen=var_steps)  # 存最近3帧 x0_p（或你想用的度量）
                ...
                for t in range(1, steps + 1):
                    actual_steps = t
                    # 1) 前向：与原逻辑一致（CFG / 温度 / gumbel / 禁特殊token / suffix锁定）
                    if cfg_scale > 0.0:
                        un_embeds = x_embeds.clone()  # (1, S, d)
                        ...
                        combined = torch.cat([x_embeds, un_embeds], dim=0)  # (2, S, d)
                        outputs = self.model(inputs_embeds=combined,
                                            system_prompt_len=system_prompt_len,
                                            image_token_length=image_token_length)
                        logits_raw_all = self.lm_head(outputs[0]).float()  # (2, S, V)
                        logits_main, logits_un = torch.chunk(logits_raw_all, 2, dim=0)
                        logits_raw = logits_un + (cfg_scale + 1.0) * (logits_main - logits_un)  # (1, S, V)
                    else:
                        outputs = self.model(inputs_embeds=x_embeds,
                                            system_prompt_len=system_prompt_len,
                                            image_token_length=image_token_length)
                        logits_raw = self.lm_head(outputs[0]).float()  # (1, S, V)

                    # 当前 mask 位：通过与 masked_embed 比较识别
                    mask_index = torch.all(torch.abs(x_embeds - masked_embed) < 1e-5, dim=2)  # (1, S)

                    # 禁用特殊 token（仅对 mask 位）
                    for token_id in [126081, 126080, 126346, 126347]:
                        logits_raw[:, :, token_id] = torch.where(
                            mask_index, torch.tensor(-float('inf'), device=device), logits_raw[:, :, token_id]
                        )

                    # 采样 logits（带 gumbel 噪声），但置信度与排序底分数都用 logits_raw
                    logits_with_noise = self.add_gumbel_noise(logits_raw, temperature=temperature)  # (1, S, V)
                    x0 = torch.argmax(logits_with_noise, dim=-1)  # (1, S)

                    # 原始置信度（用于方差与≥0.85门槛；与排序衰减解耦）
                    p = torch.softmax(logits_raw.to(torch.float64), dim=-1)  # (1, S, V)
                    x0_p = torch.gather(p, dim=-1, index=x0.unsqueeze(-1)).squeeze(-1).to(torch.float32)  # (1, S)

                    # 2) allow_mask：仅生成段 & 非 suffix &（若已发现 stop）仅 stop 之前
                    allow_mask = mask_index.clone()
                    left_mask = torch.zeros_like(allow_mask)
                    left_mask[:, gen_start:gen_end] = True
                    allow_mask = allow_mask & left_mask
                    if suffix_len > 0:
                        allow_mask[:, -suffix_len:] = False
                    if found_stop_seq:
                        allow_mask[:, stop_position:] = False

                    if not allow_mask.any():
                        break
                    
                    # 3) 更新置信度历史窗口
                    p_hist.append(x0_p.detach())

                    # 4) 排序分数：top1_raw_logit * exp(-λ i)
                    top1_raw = torch.gather(logits_raw, dim=-1, index=x0.unsqueeze(-1)).squeeze(-1).to(torch.float32)  # (1, S)
                    score_adj = top1_raw * torch.exp(-lambda_decay * i_offset)  # (1, S)
                    score_adj_masked = torch.where(allow_mask, score_adj, torch.tensor(-float('inf'), device=device))

                    # 5) 选择待解码集合
                    to_decode_mask = torch.zeros_like(allow_mask)

                    if t <= var_steps:
                        # 保守期：按衰减分数选 k_pre
                        remain = int(allow_mask.sum().item())
                        take_k = min(k_pre, remain)
                        if take_k > 0:
                            _, topk_idx = torch.topk(score_adj_masked, k=take_k, dim=1)
                            to_decode_mask.scatter_(1, topk_idx, True)
                    else:
                        # 方差 + 置信度（窗口需满 3）
                        if len(p_hist) == var_steps:
                            stack_p = torch.stack(list(p_hist), dim=0)  # (3, 1, S)
                            var_map = stack_p.var(dim=0, unbiased=False).to(torch.float32)  # (1, S)
                            stable = (var_map < var_thresh) & (x0_p >= conf_thresh) & allow_mask
                            to_decode_mask = stable.clone()
                        else:
                            # 理论上不会进入（t>3 时 len=3）
                            to_decode_mask = torch.zeros_like(allow_mask)

                    # 6) 进度对齐（强制补充：按衰减分数 score_adj 选）
                    required = min(gen_length, avg_per_step * t)
                    decoded_total = int(finalized_mask[:, gen_start:gen_end].sum().item())
                    current_pick = int(to_decode_mask.sum().item())
                    need = required - decoded_total - current_pick
                    if need > 0:
                        remain_mask = allow_mask & (~to_decode_mask)
                        if remain_mask.any():
                            score_adj_remain = torch.where(remain_mask, score_adj, torch.tensor(-float('inf'), device=device))
                            take_k = min(need, int(remain_mask.sum().item()))
                            if take_k > 0:
                                _, topk_idx = torch.topk(score_adj_remain, k=take_k, dim=1)
                                to_decode_mask.scatter_(1, topk_idx, True)
                    
                    # ---------- 7) 写回（仅在 to_decode 处最终确定） ----------
                    if to_decode_mask.any():
                        x0_embeds = self.model.embed_tokens(x0)  # (1, S, d)
                        # 仅在选中的位置写回
                        x_embeds[to_decode_mask] = x0_embeds[to_decode_mask]
                        x[to_decode_mask] = x0[to_decode_mask]
                        finalized_mask |= to_decode_mask

                    # ---------- 8) stop_tokens 检查（保持与原代码一致） ----------
                    if stopping_criteria is not None:
                        # 仅检查生成段（不含 suffix）
                        generated_part = x[0, gen_start:gen_end]  # (gen_length,)
                        # 扫描最早 stop
                        earliest_pos = None
                    ...
                    if found_stop_seq:
                        ...
                        pre_stop_masks = mask_index[0, gen_start:stop_position]
                        if not pre_stop_masks.any():
                            break
                
                print(f"actual_steps with fast-sampling is: {actual_steps}")
                ...
                for num_block in range(num_blocks):
                    ...
                    for i in range(steps):
                        ...
                        # Handle CFG, Classifier-Free Guidance, 构造一份“无条件”输入
                        if cfg_scale > 0.:
                            ...
                            # Forward pass
                            outputs = self.model(inputs_embeds=combined_embeds, system_prompt_len=system_prompt_len, image_token_length=image_token_length)
                            ...
                            # Forward pass
                            # outputs[0] is the last hidden states: [batch_size, seq_len, hidden_dim]
                            # logits is [batch_size, seq_len, vocab_size]
                            outputs = self.model(inputs_embeds=x_embeds, system_prompt_len=system_prompt_len, image_token_length=image_token_length)
                            logits = self.lm_head(outputs[0]).float()
                        
                        PRUNED_STEPS_NUMS += 1
                        if START_COMPRESSION_MODE and PRUNED_STEPS_NUMS == 12:
                            # pdb.set_trace()
                            output_visual = outputs[0][0][system_prompt_len.item():system_prompt_len.item() + image_token_length.item(), :]

                            seq_len, hidden_dim = output_visual.shape

                            k_keep = round(keep_ratio * seq_len)

                            pivot_token_idx = torch.randint(0, seq_len, ()).to(output_visual.device)

                            with torch.no_grad():
                                # start = time.perf_counter()

                                def min_max_greedy_search(
                                        image_feature_flat: torch.Tensor,
                                        k_keep: int,
                                        pivot_token_idx: int) -> torch.LongTensor:
                                    """
                                    Min-Max Greedy search

                                    Args:
                                        image_feature_flat : Tensor[n, d]   # visual features
                                        k_keep             : int           # retain token nums

                                    Returns:
                                        keep_idx           : LongTensor[k_keep]
                                    """
                                    n, _ = image_feature_flat.shape
                                    if k_keep < 1 or k_keep > n:
                                        raise ValueError(f"k_keep should bewteen 1 and {n} ，but it's {k_keep}")

                                    x = F.normalize(image_feature_flat, p=2, dim=-1)

                                    # plot
                                    #similarity_matrix = torch.matmul(x, x.transpose(0, 1))
                                    #print(similarity_matrix.shape)
                                    #self.plot_similarity(similarity_matrix)

                                    kept = [pivot_token_idx]

                                    max_sim = x @ x[pivot_token_idx]  # [n]
                                    max_sim[pivot_token_idx] = float('inf')

                                    for _ in range(k_keep - 1):
                                        cand = torch.argmin(max_sim)
                                        kept.append(cand.item())

                                        sim_new = x @ x[cand]           # [n]
                                        max_sim = torch.maximum(max_sim, sim_new)
                                        max_sim[cand] = float('inf')

                                    keep_idx = torch.tensor(kept, device=image_feature_flat.device, dtype=torch.long)
                                    keep_idx = torch.unique(keep_idx, sorted=True)

                                    return keep_idx

                                keep_idx = min_max_greedy_search(
                                    output_visual, k_keep, pivot_token_idx
                                    )

                            system_prompt_len = system_prompt_len.to(x_embeds.device)
                            image_token_length = image_token_length.to(x_embeds.device)

                            x_embeds_prompt = x_embeds[:, :system_prompt_len, :]
                            x_embeds_visual = x_embeds[:, system_prompt_len:system_prompt_len+image_token_length, :]
                            x_embeds_rest = x_embeds[:, system_prompt_len+image_token_length:, :]

                            x_prompt = x[:, :system_prompt_len]
                            x_visual = x[:, system_prompt_len:system_prompt_len+image_token_length]
                            x_rest = x[:, system_prompt_len+image_token_length:]

                            inputs_embeds_prompt = inputs_embeds[:, :system_prompt_len, :]
                            inputs_embeds_visual = inputs_embeds[:, system_prompt_len:system_prompt_len+image_token_length, :]
                            inputs_embeds_rest = inputs_embeds[:, system_prompt_len+image_token_length:, :]

                            logits_prompt = logits[:, :system_prompt_len, :]
                            logits_visual = logits[:, system_prompt_len:system_prompt_len+image_token_length, :]
                            logits_rest = logits[:, system_prompt_len+image_token_length:, :]

                            keep_idx = keep_idx.to(dtype=torch.long, device=x.device)

                            # 1) embeddings: [B, V, D]
                            x_embeds_visual = torch.index_select(x_embeds_visual, dim=1, index=keep_idx)
                            inputs_embeds_visual = torch.index_select(inputs_embeds_visual, dim=1, index=keep_idx)
                            logits_visual = torch.index_select(logits_visual, dim=1, index=keep_idx)

                            # 2) token ids: [B, V]
                            x_visual = torch.index_select(x_visual, dim=1, index=keep_idx)

                            x_embeds = torch.cat([x_embeds_prompt, x_embeds_visual, x_embeds_rest], dim=1)
                            x = torch.cat([x_prompt, x_visual, x_rest], dim=1)
                            inputs_embeds = torch.cat([inputs_embeds_prompt, inputs_embeds_visual, inputs_embeds_rest], dim=1)
                            logits = torch.cat([logits_prompt, logits_visual, logits_rest], dim=1)
                            ...
                            block_start = inputs_embeds.shape[1] + num_block * block_length
                            block_end = inputs_embeds.shape[1] + (num_block + 1) * block_length
                            block_end = min(block_end, x.shape[1])
                        ...
                        # row = x0[0][-64:].detach().cpu().numpy().tolist()
                        # # 追加写入csv
                        ...
                        # row = x0_p[0][-64:].detach().cpu().numpy().tolist()
                        # # 追加写入csv
                        # with open(csv_file, mode="a", newline="", encoding="utf-8") as f:
                        #     writer = csv.writer(f)
                        #     writer.writerow(row)
                        ...
                        # 为当前候选 token 映射 embedding，并仅在掩码位替换；非掩码位保持原值。
                        x0_embeds = self.model.embed_tokens(x0) # shape (1, l + gen_length + suffix_len, d), one-hot投射到embedding的表示
                        ...
                        # 选中的top-k的token用来替换原输出里mask位置的token
                ...
                print(f"actual_steps is: {actual_steps}")

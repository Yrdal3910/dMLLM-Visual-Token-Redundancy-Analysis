...
START_COMPRESSION_MODE = False
PRUNING_LOCATION_RATIO = [0.25, 0.5, 0.75]
PRUNED_LAYER_NUMS = 0
PRUNED_STEPS_NUMS = 0
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, system_prompt_len: Optional[torch.Tensor] = None, image_token_length: Optional[torch.Tensor] = None, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        '''
        global PRUNED_STEPS_NUMS, PRUNING_LOCATION_RATIO
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            assert steps % num_blocks == 0
            self.pruning_location = torch.tensor([int(x * steps) for x in PRUNING_LOCATION_RATIO if (x * steps).is_integer()], dtype=torch.int32)
            self.image_token_length = image_token_length.item()
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    if cfg_scale > 0.:
                        ...
                        logits = self.lm_head(outputs[0]).float()
                    if START_COMPRESSION_MODE and (num_block * steps + i + 1) in self.pruning_location:
                        output_visual = outputs[0][0][system_prompt_len:system_prompt_len + image_token_length, :]
                        seq_len = self.image_token_length
                        prune_ratio = (num_block * steps + i + 1) / (num_blocks * steps)
                        k_prune = round(prune_ratio * seq_len) - (seq_len - output_visual.shape[0])
                        with torch.no_grad():
                            def drop_by_1nn_similarity(X: torch.Tensor, k_prune: int):
                                """
                                X: Visual token vectors with shape (n, d).
                                drop_ratio: Fraction of tokens to remove.
                                Returns: Indices of the retained tokens.
                                """
                                ...
                                m_drop = int(k_prune)
                                drop_idx = torch.topk(nn_sim, k=m_drop, largest=True).indices
                                keep_mask = torch.ones(n, dtype=torch.bool, device=X.device)
                                keep_mask[drop_idx] = False
                                keep_idx = torch.nonzero(keep_mask, as_tuple=False).squeeze(1)

                                return keep_idx
                            keep_idx = drop_by_1nn_similarity(output_visual, k_prune)
                        ...
                        image_token_length = torch.tensor(image_token_length.item() - k_prune)

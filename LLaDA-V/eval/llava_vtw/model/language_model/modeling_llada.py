...
# VTw schedule. Both values are zero-based:
# - step 47 means the 48th denoising/model forward pass.
# - layer 15 means removing visual tokens after the 16th Transformer layer.
VTW_PRUNING_STEP = 47
VTW_PRUNING_LAYER = 15


def get_vtw_target_layer_call(num_hidden_layers):
    """Return the cumulative layer count immediately after VTw pruning."""
    return (
        VTW_PRUNING_STEP * num_hidden_layers
        + VTW_PRUNING_LAYER
        + 1
    )


def is_vtw_pruning_point(layer_call_count, layer_idx, num_hidden_layers):
    """Return whether the current layer output should be pruned by VTw."""
    return (
        layer_call_count == get_vtw_target_layer_call(num_hidden_layers)
        and layer_idx == VTW_PRUNING_LAYER
    )
...
class LLaDAAttention(nn.Module):
    ...
    def adjust_position_ids_after_pruning(self, position_ids, prune_num):
        """Shorten position IDs for layers after the VTw removal point."""
        global PRUNED_LAYER_NUMS, PRUNED_STEPS_NUMS

        if (
            PRUNED_LAYER_NUMS
            == get_vtw_target_layer_call(self.num_hidden_layers)
            and PRUNED_STEPS_NUMS == VTW_PRUNING_STEP
        ):
            return adjust_position_ids(position_ids, prune_num)
        return position_ids
    ...
    def forward(
        ...
        **kwargs,
        ...
        position_ids = self.adjust_position_ids_after_pruning(
            position_ids,
            prune_num,
        )
        ...
        return attn_output, attn_weights, past_key_value, position_ids
...
class LLaDASdpaAttention(LLaDAAttention):
    ...
    def forward(
        ...
        return attn_output, None, past_key_value, position_ids
...
class LLaDADecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value, position_ids = self.self_attn(
            hidden_states=hidden_states,
            prune_num=prune_num,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        global PRUNED_LAYER_NUMS

        # Count completed Transformer layers across denoising passes. The
        # counter is reset after each generated sample.
        PRUNED_LAYER_NUMS += 1

        if is_vtw_pruning_point(
            PRUNED_LAYER_NUMS,
            self.layer_idx,
            ...
            hidden_states_rest = hidden_states[:, system_prompt_len + visual_seq_len:, :]

            hidden_states = torch.cat([hidden_states_prompt, hidden_states_rest], dim=1)

            prune_num = torch.full((batch_size,), visual_seq_len, device=device)
        residual = hidden_states
        ...
        **kwargs,
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                def custom_forward(hidden_states, prune_num, position_ids):
                    outputs, next_prune_num, next_position_ids = decoder_layer(
                        hidden_states,
                        prune_num=prune_num,
                    ...
                    return (*outputs, next_prune_num, next_position_ids)

                checkpoint_outputs = self._gradient_checkpointing_func(
                    custom_forward,
                    hidden_states,
                    prune_num,
                    position_ids,
                )
                layer_outputs = checkpoint_outputs[:-2]
                prune_num = checkpoint_outputs[-2]
                position_ids = checkpoint_outputs[-1]
            else:
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    # One complete model forward corresponds to one denoising
                    # step. VTw removes visual tokens inside the configured
                    # layer; after the forward returns, remove the same tokens
                    # from all persistent generation states.
                    PRUNED_STEPS_NUMS += 1
                    if PRUNED_STEPS_NUMS == VTW_PRUNING_STEP + 1:
                        ...
                        x_embeds = torch.cat([x_embeds_prompt, x_embeds_rest], dim=1)
                        x = torch.cat([x_prompt, x_rest], dim=1)
                        inputs_embeds = torch.cat([inputs_embeds_prompt, inputs_embeds_rest], dim=1)

...
# SparseVLM pruning schedule. Both values are zero-based:
# - step 47 means the 48th denoising/model forward pass.
# - layer 6 means pruning at the seventh Transformer layer.
SPARSEVLM_PRUNING_STEP = 47
SPARSEVLM_PRUNING_LAYER = 6

# The layer-level selection mask and merged sparse token are reused after the
# model forward to keep token IDs, logits, and persistent embeddings aligned.
VISUAL_INDICES = 0
MERGE_SPARSE_TOKEN = 0

from .utils import *
from .score import *


def is_sparsevlm_pruning_point(step_idx, layer_idx, pruning_locations):
    """Return whether SparseVLM should prune at this step and layer."""
    return (
        step_idx == SPARSEVLM_PRUNING_STEP
        and layer_idx in pruning_locations
    )
...
class LLaDASdpaAttention(LLaDAAttention):
    ...
    def forward(
        ...
        return attn_output, None, past_key_value, attn_logits, position_ids
...
class LLaDADecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # LLaDAModel decides whether this is the configured SparseVLM pruning
        # point and passes sparse_layer=True only for that layer invocation.
        if sparse_layer:
            hidden_states, self_attn_weights, present_key_value, relation_vis_text, position_ids  = self.self_attn(
                hidden_states=hidden_states,
                prune_num=prune_num,
                policy=policy,
                attention_mask=attention_mask,
            ...
            prune_num = torch.full((hidden_states.shape[0],), int(v_token_num * PRUNING_RATIO), device=prune_num.device)
            ...
            hidden_states, self_attn_weights, present_key_value, attn_logits, position_ids = self.self_attn(
                hidden_states=hidden_states,
                prune_num=prune_num,
                policy=policy,
                attention_mask=attention_mask,
...
class LLaDAModel(LLaDAPreTrainedModel):
    ...
    def __init__(self, config: LLaDAConfig, pruning_loc=None):
        ...
        # Keep pruning_loc override support while using the global schedule by
        # default. A tuple avoids a mutable default argument.
        self.pruning_loc = (
            (SPARSEVLM_PRUNING_LAYER,)
            if pruning_loc is None
            else tuple(pruning_loc)
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.visual_seq_len = image_token_length.to(system_prompt_len.device)
        ...
        device = self.system_prompt_len.device

        v_token_start = self.system_prompt_len.to(device)
        text_token_start = (v_token_start + self.visual_seq_len).to(device)
        v_token_num = self.visual_seq_len.to(device)

        if self.system_prompt_len.item() != 0 and hidden_states.shape[1] != 1:
        ...
        global PRUNING_RATIO, VISUAL_INDICES, MERGE_SPARSE_TOKEN, PRUNED_STEPS_NUMS
        prune_num = torch.full((batch_size,), 0, device=self.visual_seq_len.device)
        for layer_idx, decoder_layer in enumerate(self.layers):
            # Apply SparseVLM only at the configured denoising step and
            # Transformer layer. Text-only and single-token passes are skipped.
            if (
                is_sparsevlm_pruning_point(
                    PRUNED_STEPS_NUMS,
                    layer_idx,
                    self.pruning_loc,
                )
                and self.system_prompt_len.item() != 0
                ...
                # Inference
                    ...
                    # relation_vis_text is always appended as the final element
                    # by the sparse DecoderLayer branch. Using -1 is stable
                    # regardless of output_attentions or use_cache.
                    attn_logits = layer_outputs[-1]

                    pred_score_vis, s_flag, relation_vis_text = attn_postprocess_topk(attn_logits, v_token_start, v_token_num, text_token_start, t_token_idx, layer_idx, self.visual_seq_len.item() * PRUNING_RATIO) # B, L_v
                    VISUAL_INDICES = pred_score_vis
                    ...
                    for batch in range(len(self.system_prompt_len)):
                        # keep pre prompt
                        prompt_length = self.system_prompt_len[batch]
                        policy[batch,:prompt_length,] = 1
                        # keep question
                        text_token_start = prompt_length + self.visual_seq_len
                    ...
                    if s_flag and total_sparse_token_idx.shape[1]>0:
                        ...
                        MERGE_SPARSE_TOKEN =  merge_sparse_token
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, system_prompt_len: Optional[torch.Tensor] = None, image_token_length: Optional[torch.Tensor] = None, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        '''
        global PRUNED_STEPS_NUMS, PRUNING_RATIO, VISUAL_INDICES, MERGE_SPARSE_TOKEN
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    # A complete model forward is one denoising step. SparseVLM
                    # selects/merges visual tokens inside the model while this
                    # counter is SPARSEVLM_PRUNING_STEP. After the forward
                    # returns, synchronize all persistent generation states.
                    PRUNED_STEPS_NUMS += 1
                    if PRUNED_STEPS_NUMS == SPARSEVLM_PRUNING_STEP + 1:
                        ...
                        vis_mask = (VISUAL_INDICES > 0)
                        keep_idx = vis_mask[0].nonzero(as_tuple=True)[0].long()
                        idx_expanded = keep_idx.view(1, -1, 1).expand(1, keep_idx.numel(), x_embeds_visual.size(-1))
                        x_embeds_visual = x_embeds_visual.gather(1, idx_expanded)
                        inputs_embeds_visual= inputs_embeds_visual.gather(1, idx_expanded)

                        x_visual = x_visual.gather(1, keep_idx.view(1, -1))
                        logits_pre = logits[:, :x_prompt.shape[1] + x_visual.shape[1], :]
                        logits_post = logits[:, x_prompt.shape[1] + x_visual.shape[1] + MERGE_SPARSE_TOKEN.shape[1]:, :]
                        logits = torch.cat([logits_pre, logits_post], dim=1)
            ...
            # SparseVLM state is sample-local and must not leak into the next
            # generation request.
            PRUNED_STEPS_NUMS = 0
            VISUAL_INDICES = 0
            MERGE_SPARSE_TOKEN = 0
